...
<div class="min-h-screen">
    <header class="border-b border-slate-800 bg-slate-900/50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <h1 class="text-xl sm:text-2xl font-semibold text-white">Edit {{ $user->name }}</h1>
        </div>
    </header>

    <main class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form method="POST" action="{{ route('users.update', $user) }}" class="rounded-xl border border-slate-800 bg-slate-900 p-6 space-y-4">
            @csrf
            @method('PUT')
